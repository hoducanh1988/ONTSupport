...
namespace ONTWiFiMaster.Function.Protocol {
    ...
    public class Serial<T> where T : class, new() {
        ...
        public Serial(T _t, string _portname) {
            this.t = _t;
        ...
        public string Read() {
            string data = "";
            try {
                data = this._serialport.ReadExisting();
            }
            catch (Exception ex) {
                data = ex.ToString();
            }
            PropertyInfo pi = t.GetType().GetProperty("logUart");
